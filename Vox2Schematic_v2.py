...
def performOLD(level, box, options):
	...
	renderModels(level,box,VOXchunks,matlist,colours)
...
def renderModels(level,box,chunks,matlist,colours):
	MATERIAL = (1,0)
	for (type,data) in chunks:
		print type
		if type == "XYZI":
			for (x,y,z,i) in data:
				if i in matlist:
					(r,g,b,a) = colours[(i-1)%256]
					(block, damage) = closestMaterial(DefaultColourPallet, r,g,b)
...
#	print chunks
#	print matlist
	return (version,numModels,chunks,matlist)